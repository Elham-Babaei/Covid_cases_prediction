...
ggplot(d.train, aes(Variation_cases,Intensive_care ))+
  geom_point()+
  ggtitle("Polynomial of degree 2")+
  stat_smooth(method= glm, formula= y~ poly(x,2, raw=TRUE))

ggplot(d.train, aes(Variation_cases,Intensive_care ))+
  geom_point()+
   ggtitle("Polynomial of degree 3")+
...
#par(mfrow=c(2,2))
plot(d.train$Date, d.train$Intensive_care, xlab = "Date", ylab="Fitted values")
points(d.train$Date, model.glm$fitted.values,col="blue", pch=19,bg="blue")
points(d.train$Date, model.glm.nb$fitted.values, col="red", pch=20, bg="red")
legend("bottomright", c("GLM","NB"), pch=c(19,20), col=c("blue", "red"))
...
points(d.train$Variation_cases, model.glm.nb$fitted.values, col="red", pch=20, bg="red")
legend("bottomright", c("GLM","NB"), pch=c(19,20), col=c("blue", "red"))
...
points(d.train$Death_today, model.glm.nb$fitted.values, col="red", pch=20, bg="red")
legend("bottomright", c("GLM","NB"), pch=c(19,20), col=c("blue", "red"))
...
points(d.train$Test_today, model.glm.nb$fitted.values, col="red", pch=20, bg="red")
legend("bottomright", c("GLM","NB"), pch=c(19,20), col=c("blue", "red"))
...
library(ggplot2)
 ggplot()+ 
   ggtitle("Predictions from 14th to 23rd Jan. 2021 before adding new covariates")+
...
pred.glm.all <- predict(model.glm, data, type= "response")
pred.glm.quasi.all <- predict(model.glm.quasi, data, type= "response")
pred.glm.nb.all <- predict(model.glm.nb, data, type= "response")
pred.gam.all <- predict(model.gam, data, type= "response")
pred.rf.all <- predict(model.rf, data, type = "response")
#cbind.data.frame(data$Intensive_care,pred.glm.all,pred.glm.quasi.all,pred.glm.nb.all,pred.gam.all,pred.rf.all )
 
 
 library(ggplot2)
 ggplot()+ 
   ggtitle("Predictions from 1st Sep. to 23rd Jan. 2021 before adding new covariates")+
   geom_line(data=data, aes(x=Date, y=Intensive_care, color="actual"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.glm.all, color="Poisson"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.glm.quasi.all, color="Quasi Poisson"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.glm.nb.all, color="Negative Binomial"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.gam.all, color="GAM"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.rf.all, color= "RF"), size=1)+
    geom_vline(xintercept = as.numeric(as.Date("2021-01-14")), linetype=4, size=1)+
...
library(ggplot2)
 ggplot()+ 
   ggtitle("Predictions from 14th to 23rd Jan. 2021 after adding new covariates")+
    geom_line(data=d.test, aes(x=Date, y=Intensive_care, color="actual"), size=1)+
...
pred.glm.all <- predict(model.glm1, data, type= "response")
pred.glm.quasi.all <- predict(model.glm.quasi1, data, type= "response")
pred.glm.nb.all <- predict(model.glm.nb1, data, type= "response")
pred.gam.all <- predict(model.gam1, data, type= "response")
pred.rf.all <- predict(model.rf1, data, type = "response")
#cbind.data.frame(data$Intensive_care,pred.glm.all,pred.glm.quasi.all,pred.glm.nb.all,pred.gam.all,pred.rf.all )
 
 
 library(ggplot2)
 ggplot()+ 
   ggtitle("Predictions from 1st Sep. to 23rd Jan. 2021 after adding new covariates")+
   geom_line(data=data, aes(x=Date, y=Intensive_care, color="actual"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.glm.all, color="Poisson"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.glm.quasi.all, color="Quasi Poisson"), size=1)+
    geom_line(data=data, aes(x=Date, y=pred.glm.nb.all, color="Negative Binomial"), size=1)+
